...
//#endif
    ...
    self.deleteControl.image = [UIImage imageNamed:@"stamp_del.png"];
    ...
    self.resizingControl.image = [UIImage imageNamed:@"stamp_ex.png"];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    */
    ...
    if ([self.stickerViewDelegate respondsToSelector:@selector(stickerViewTouchBegin:)])
    {
        // 編集中でなければ重なっている領域でもスタンプを貼り付ける
        // 貼り付けずに選択にする
        if([self isEditingHandlesHidden]){
            [self.stickerViewDelegate stickerViewTouchBegin:self];
        }
    }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    /*
    // 画像から外れたら画像の中央に戻す処理
    if (CGRectContainsPoint(self.rangeRect, self.center)){
        //NSLog(@"ok");
    }else{
        //NSLog(@"ng");
        self.center = CGPointMake(
                                  (self.rangeRect.origin.x + self.rangeRect.size.width)/2,
                                  (self.rangeRect.origin.y + self.rangeRect.size.height)/2
                                  );
        return;
    }
    */
